...
DB_PATH = os.getenv("SQLITE_DB_PATH", os.path.join(BASE_DIR, "chat_history.db"))
...
def init_db() -> None:
    with _get_connection() as conn:
        ...
        conn.execute(
            ...
            ON messages(user_id, chat_session_id, created_at)
            """
        )
...
def save_thread(user_id: str, chat_session_id: str) -> None:
    with _get_connection() as conn:
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO chat_sessions(user_id, chat_session_id)
            VALUES (?, ?)
            """,
